...
DATABASE_NAME="${GLUE_DATABASE_NAME:-simem_silver}"
JOB_NAME="${GLUE_JOB_NAME:-simem-bronze-to-silver}"
CRAWLER_NAME="${GLUE_CRAWLER_NAME:-simem-silver-crawler}"
SCRIPT_LOCAL_PATH="${SCRIPT_LOCAL_PATH:-${PROJECT_ROOT}/jobs/simem_bronze_to_silver.py}"
SCRIPT_S3_PATH="s3://${BUCKET}/glue/scripts/simem_bronze_to_silver.py"
SOURCE_PATH="s3://${BUCKET}/bronze/simem-data/"
TARGET_PATH="s3://${BUCKET}/silver/simem-data/"
...
if aws glue get-job --job-name "${JOB_NAME}" "${AWS_ARGS[@]}" >/dev/null 2>&1; then
  ...
  aws glue update-job \
    ...
    --job-update "{
      ...
      \"DefaultArguments\":{\"--SOURCE_PATH\":\"${SOURCE_PATH}\",\"--TARGET_PATH\":\"${TARGET_PATH}\",\"--job-language\":\"python\"},
      \"MaxRetries\":0,
      \"Timeout\":60,
      \"WorkerType\":\"G.1X\",
      \"NumberOfWorkers\":2
  ...
  aws glue create-job \
    ...
    --command "{\"Name\":\"glueetl\",\"ScriptLocation\":\"${SCRIPT_S3_PATH}\",\"PythonVersion\":\"3\"}" \
    --default-arguments "{\"--SOURCE_PATH\":\"${SOURCE_PATH}\",\"--TARGET_PATH\":\"${TARGET_PATH}\",\"--job-language\":\"python\"}" \
    --max-retries 0 \
    --timeout 60 \
    --worker-type "G.1X" \
    --number-of-workers 2 \
